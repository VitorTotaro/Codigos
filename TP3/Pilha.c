...
void mostrar(Pilha *p) {
    ...
    for (int i = p->n-1; i >= 0; i--) {
        printf("[%d] => %s ## %s ## %s ## %s ## [", 
           k, p->array[i].show_id, p->array[i].title, p->array[i].type, p->array[i].director);

        for (int j = 0; j < p->array[i].cast_size; j++) {
            printf("%s", p->array[i].cast1[j]);
            if (j < p->array[i].cast_size - 1) {
                printf(", ");
            }
        }

        printf("] ## %s ## %s ## %s ## %s ## %s ## [%s] ##\n",
            p->array[i].country, p->array[i].date, p->array[i].release_year,
            p->array[i].rating, p->array[i].duration, p->array[i].listed_in);
        k--;
    }
}
