...
int main() {
    ...
    // Teste: Verifique se o arquivo foi aberto
    ...
    while (fgets(linha, MAX_LINE, file) && total < MAX_SHOWS) {
        ...
        // Teste: Imprime o total de shows lidos
        printf("Total de shows lidos: %d\n", total);
    }

    fclose(file);

    // Teste: Verifique quantos shows foram carregados
    printf("Carregado %d shows\n", total);

    char entrada[10];
    printf("Digite um índice de show (ou 'FIM' para encerrar):\n");
