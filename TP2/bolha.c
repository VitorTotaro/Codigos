...
void bolha(Show *array, int n){
    for (int i = 0; i < (n - 1); i++) {
        for (int j = i; j < (n - 1); j++) {
            comparacoes++; // comparação do ano
            if (array[j].data.ano > array[j + 1].data.ano) {
                swap(&array[j], &array[j + 1]);
            } else if (array[j].data.ano == array[j + 1].data.ano) {
                comparacoes++; // comparação do mês
                if (array[j].data.mes > array[j + 1].data.mes) {
                    swap(&array[j], &array[j + 1]);
                } else if (array[j].data.mes == array[j + 1].data.mes) {
                    comparacoes++; // comparação do dia
                    if (array[j].data.dia > array[j + 1].data.dia) {
                        swap(&array[j], &array[j + 1]);
                    } else if (array[j].data.dia == array[j + 1].data.dia) {
                        comparacoes++; // comparação do título
...
int main(){
        ...
        // Ordenar com seleção recursiva por título
        clock_t inicio = clock();
        bolha(selecionados, n);
        clock_t fim = clock();
    
        double tempo = (double)(fim - inicio) / CLOCKS_PER_SEC;
    
        FILE *log = fopen("872284_bolha.txt", "w");
        if (log != NULL) {
