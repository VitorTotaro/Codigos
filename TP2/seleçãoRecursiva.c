...
void selecaoRecursiva(Show *array, int n, int i) {
    if (i >= n - 1) return;

    int min = i;
    for (int j = i + 1; j < n; j++) {
        comparacoes++;
        if (strcmp(array[j].title, array[min].title) < 0) {
            min = j;
        }
    }
...
int main() {
    ...
    selecaoRecursiva(selecionados, n, 0);
    clock_t fim = clock();

    double tempo = (double)(fim - inicio) / CLOCKS_PER_SEC;

    FILE *log = fopen("872284_selecaoRecursiva.txt", "w");
    if (log != NULL) {
